---
bibkey: fang2023fairness
zotero-key: WEPKQM55
title: "Fairness in Face Presentation Attack Detection"
type: preprint
creators: "Fang, Yang, Kuijper, Struc, Damer"
date: 2023-10-19
url: http://arxiv.org/abs/2209.09035
last-sync: 2026-05-03T19:53:51.861Z
---

# Fairness in Face Presentation Attack Detection

> **Fang, Yang, Kuijper, Struc, Damer** · *preprint* · 2023-10-19 · [link](http://arxiv.org/abs/2209.09035)
> Zotero key: `WEPKQM55` · Bibkey: `@fang2023fairness`

## Tags
- **type:** `conference`
- **role:** `evidence`, `state-of-the-art`
- **area:** `liveness`, `pad`, `privacy`
- **thesis:** `MULTI-LIV`, `regulatory-framework`, `state-of-the-art`
- **modality:** `face`
- **quality:** `core`
- **priority:** `high`

## Collections
- PhD_UMinho_Liveness / 40_Thesis_Manuscript / 48_RegulatoryFramework

---

## Notas (formato livre — converter para template quando possível)

Comment: Accepted at Pattern Recognition
