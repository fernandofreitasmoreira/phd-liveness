...
“In FL, the users, such as hospitals, are equipped with storage and computation capability and can locally hold and process the patients’ data, e.g., medical imaging datasets or clinical records [2]. The users iteratively train their local models (i.e., weight parameters or gradients) by taking advantage of their private data, and send the local models instead of the raw private data to a server for aggregation.” (Zheng et al., 2024, p. 1) TESTE 2
